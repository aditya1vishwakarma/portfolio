...
/* ─────────────────────────────────────────────
 * Clouds — vendored from canvasui.dev.
 *
 * A WebGL2 volumetric-ish cloud field composited over whatever it wraps.
 * Two paths:
 *
 *   • html-in-canvas (Chrome experimental `drawElementImage` / `requestPaint`)
 *     renders the children *into* a source canvas, which the shader can then
 *     fog and refract. This is the fancy path.
 *   • Everywhere else, children render as normal DOM and the cloud canvas is
 *     an absolutely-positioned, pointer-events:none overlay on top of them.
 *     `uHasContent = 0` makes the shader emit transparent-backed clouds, so
 *     the DOM underneath simply shows through.
 *
 * The overlay is inert to the pointer, so `pointermove` still reaches the
 * content div — that's what drives the wind displacement.
 *
 * Deviations from upstream: dropped the Next.js "use client" directive, and
 * memoised the `supportsHtmlInCanvas` probe (below) because it is used as a
 * `useSyncExternalStore` snapshot and was allocating a canvas per call.
 * ───────────────────────────────────────────── */
...
// Memoised: this is the `useSyncExternalStore` snapshot, which React may call
// several times per render — upstream allocated a throwaway canvas each time.
// The answer is a static browser capability, so probing once is enough.
let htmlInCanvasSupport: boolean | undefined;

export function supportsHtmlInCanvas(): boolean {
  if (htmlInCanvasSupport !== undefined) return htmlInCanvasSupport;
  if (typeof document === 'undefined') return false;
  const probe = document.createElement('canvas') as PaintableCanvas;
  const ctx = probe.getContext('2d') as ElementImageContext | null;
  htmlInCanvasSupport = Boolean(
    ctx &&
...
export function createClouds(
  ...
  /* Pointer-driven wind is DISABLED.
   *
   * Upstream lets the cursor push the cloud field around. In (Rest) that works
   * against the point — the clouds should drift on their own and the viewer
   * should be still. Uncomment these two lines (and the matching removes in
   * destroy()) to bring it back; `onPointerMove` / `onPointerLeave` above are
   * left intact so it's a pure uncomment.
   *
   * Consequence: the `wind` and `windRadius` options become inert, which is why
   * the controls panel doesn't offer them.
   */
  ...
  return {
    ...
    destroy() {
      ...
      // Paired with the disabled listeners above — uncomment both together.
...
export function Clouds({
  ...
  // Bumped on webglcontextrestored to force a full rebuild. See the effect below.
  ...
  useEffect(() => {
    ...
    /* Context loss (added on top of upstream).
     *
     * Safari drops WebGL contexts far more eagerly than Chrome — a backgrounded
     * tab, memory pressure, or a GPU switch on a dual-GPU Mac will all do it.
     * Two things are needed to survive that:
     *
     *   • preventDefault() on `webglcontextlost`. Without it the browser never
     *     even attempts restoration, and the layer stays blank for the rest of
     *     the session while the DOM underneath carries on none the wiser.
     *   • A full rebuild on `webglcontextrestored`. Every texture, buffer,
     *     shader and program from the dead context is invalid, so re-creating
     *     the instance is the only correct response — there's nothing to salvage.
     *
     * Tearing down on loss also stops the old render loop, which would
     * otherwise keep spinning through no-op GL calls until restoration.
     */
    ...
    output.addEventListener('webglcontextrestored', onRestored);
    ...
    return () => {
      ...
      output.removeEventListener('webglcontextrestored', onRestored);
      instanceRef.current?.destroy();
      instanceRef.current = null;
    };
  }, [initialOptions, native, generation]);
